...
class ViewUserWeek_overview(View):
    HTML_FILE = "week_overview_js.html"
    URL_LINK = "week_overview_js"
    ...
    def get(self, request:HttpRequest):#{
        ...
        weekhours = UsersWorkTimes.objects.filter(user= request.user).filter(date__gte= date_start).filter(date__lte= date_end).values(
            "id","date","travel_start", "travel_end", "work_start", "work_end", "workimes_standard__description", "project_machine", "netzplan_vorgang")
        json_weekhours = list(weekhours)
        #json_weekhours = [model_to_dict(i) for i in  weekhours]

        context = {"iso_date_list" :  iso_date_list, "settings" : settings, "json_weekhours" : json_weekhours}
        return render(request= request, template_name= self.HTML_FILE, context= context)
    #}
    def post(self, request:HttpRequest):#{
...
class ViewUserCurrentDay_modify(View):
    ...
    def get(self, request:HttpRequest, id:int, cd:str):#{
        ...
        context = {"currententry" : query_set, "form" : form, "id" : id, "cd" : cd}
        return render(request= request, template_name= self.HTML_FILE, context= context)
    #}
    ...
    def post(self, request:HttpRequest, id:int, cd:str):#{
        ...
        if form.is_valid():#[]
            ...
            if form.cleaned_data["travel_start"]:
                currententry.travel_start = form.cleaned_data["travel_start"]

            if form.cleaned_data["travel_end"]:
                currententry.travel_end = form.cleaned_data["travel_end"]

            if form.cleaned_data["work_start"]:
                currententry.work_start = form.cleaned_data["work_start"]

            if form.cleaned_data["work_end"]:
                currententry.work_end = form.cleaned_data["work_end"]

            if form.cleaned_data["worktimes_standard"]:
                currententry.workimes_standard = form.cleaned_data["worktimes_standard"]

            if form.cleaned_data["project_machine"]:
                currententry.project_machine = str(form.cleaned_data["project_machine"])

            if form.cleaned_data["netzplan_vorgang"]:
                currententry.netzplan_vorgang = str(form.cleaned_data["netzplan_vorgang"])

            currententry.save()
            return HttpResponseRedirect(reverse(self.URL_LINK, args=[id, cd]))
        #]
        else:#[
